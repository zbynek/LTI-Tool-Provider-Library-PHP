 ...
 */
    public function loadToolConsumer($consumer)
    {
        ...
        $rsConsumer = $this->sql_query($sql);
        if ($rsConsumer) {
            while ($row = $this->sql_fetch_object($rsConsumer)) {
            ...
            $this->sql_free_result($rsConsumer);
        }
 ...
 */
    public function saveToolConsumer($consumer)
    {
        ...
        if ($ok) {
            if (empty($id)) {
                $consumer->setRecordId($this->sql_insert_id());
...
###
    public function getToolConsumers()
    {
        ...
        $sql = 'SELECT consumer_pk, consumer_key, consumer_key, name, secret, lti_version, consumer_name, consumer_version, consumer_guid, ' .
               ...
               "FROM {$this->dbTableNamePrefix}" . DataConnector::CONSUMER_TABLE_NAME . ' ' .
               'ORDER BY name';
        $rsConsumers = $this->sql_query($sql);
        if ($rsConsumers) {
            while ($row = $this->sql_fetch_object($rsConsumers)) {
            ...
            $this->sql_free_result($rsConsumers);
        }
 ...
 */
    public function loadContext($context)
    {
        ...
        $rs_context = $this->sql_query($sql);
        if ($rs_context) {
            $row = $this->sql_fetch_object($rs_context);
            if ($row) {
 ...
 */
    public function saveContext($context)
    {
        ...
        if ($ok) {
            if (empty($id)) {
                $context->setRecordId($this->sql_insert_id());
 ...
 */
    public function loadResourceLink($resourceLink)
    {
        ...
        $rsContext = $this->sql_query($sql);
        if ($rsContext) {
            $row = $this->sql_fetch_object($rsContext);
            if ($row) {
 ...
 */
    public function saveResourceLink($resourceLink) {
        ...
        if ($ok) {
            if (empty($id)) {
                $resourceLink->setRecordId($this->sql_insert_id());
 ...
 */
    public function getUserResultSourcedIDsResourceLink($resourceLink, $localOnly, $idScope)
    {
        ...
        $rsUser = $this->sql_query($sql);
        if ($rsUser) {
            while ($row = $this->sql_fetch_object($rsUser)) {
 ...
 */
    public function getSharesResourceLink($resourceLink)
    {
        ...
        $sql = sprintf('SELECT consumer_pk, resource_link_pk, share_approved ' .
                       ...
                       $resourceLink->getRecordId());
        $rsShare = $this->sql_query($sql);
        if ($rsShare) {
            while ($row = $this->sql_fetch_object($rsShare)) {
...
// Load the nonce
        ...
        $rs_nonce = $this->sql_query($sql);
        if ($rs_nonce) {
            $row = $this->sql_fetch_object($rs_nonce);
            if ($row === false) {
                $ok = false;
            }
        }
...
// Load share key
        $id = $this->sql_real_escape_string($shareKey->getId());
        ...
        $rsShareKey = $this->sql_query($sql);
        if ($rsShareKey) {
            $row = $this->sql_fetch_object($rsShareKey);
 ...
 */
    public function loadUser($user)
    {
        ...
        $rsUser = $this->sql_query($sql);
        if ($rsUser) {
            $row = $this->sql_fetch_object($rsUser);
            if ($row) {
                $user->setRecordId(intval($row->user_pk));
 ...
 */
    public function saveUser($user)
    {
        ...
        if ($ok) {
            if (is_null($user->created)) {
                $user->setRecordId($this->sql_insert_id());
                $user->created = $time;
            }
            $user->updated = $time;
        }
